...
if git rev-parse --git-dir >/dev/null 2>&1; then
  ...
  if [ "$LOCAL" = "$REMOTE" ]; then
    say "✓ Source already up to date ($(git rev-parse --short HEAD))."
  else
...
if command -v docker >/dev/null 2>&1 && docker compose ps >/dev/null 2>&1 \
  ...
  say "✓ Updated. Logs:  docker compose logs -f app"
else
  say "→ Native install — installing dependencies…"
  npm install
  ( cd web && npm install )
  say "✓ Updated. Restart:  npm run server   (and in another shell:  cd web && npm run dev)"
fi
